...
class BranchPredictorPAg {
    ...
    public:
        ...
        inline bool predict(Address branchPc, bool taken) {
            ...
            (A2)
            if (taken)
                if (prediction == take)         // correct prediction :
                    state := 3;                 // @state 3 -> stay | @state 2 -> state 3
                else                            // wrong prediction :
                    state := state + 1;         // @state 1 -> state 2 | @state 0 -> state 1 
            else
                if (prediction == true)         // wrong prediction :
                    state := state - 1;         // @state 2 -> state 1 | @state 3 -> state 2
                else                            // correct prediction :
                    state := state 0;           // @state 0 -> stay | @state 1 -> state 0
            
            (A3)
            if (taken)
                if (prediction == take OR state == 1)    
                    state := 3;                 // @state 3 -> stay | @state 2 -> state 3 | @state 1 -> state 0
                else       
                    state := state + 1;         // @state 0 -> state 1
            else
                if (prediction == true AND state == 3)         
                    state := state - 1;         // @state 3 -> state 2 
                else                            
                    state := state 0;           // @state 0 -> stay | @state 1 -> state 0 | @state 2 -> state 0

            */
            if (useA2) {
                pht[phtIdx] = taken? (pred? 3 : (pht[phtIdx]+1)) : (pred? (pht[phtIdx]-1) : 0); //2-bit saturating counter
            } else {
                // TODO: Please implement Automaton 3 for update
                pht[phtIdx] = taken? ((pred | (phtIdx==1))? 3 : (pht[phtIdx]+1)) : ((pred & (phtIdx==3))? (pht[phtIdx]-1) : 0);
            }
